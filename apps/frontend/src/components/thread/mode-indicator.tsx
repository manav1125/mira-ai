...
export const ModeIndicator = memo(function ModeIndicator() {
  ...
  const basicModel = useMemo(
    () => modelOptions.find((m) => m.id === 'kortix/basic' || m.label === 'Mira Basic'),
    [modelOptions]
  );
  
  const powerModel = useMemo(
    () => modelOptions.find((m) => m.id === 'kortix/power' || m.label === 'Mira Advanced Mode'),
    [modelOptions]
  );
  ...
  const otherModels = useMemo(() => {
    return modelOptions.filter(
      (m) => m.id !== 'kortix/basic' && m.id !== 'kortix/power' && 
             m.label !== 'Mira Basic' && m.label !== 'Mira Advanced Mode'
    ...
    if (powerModel) {
      if (canAccessPower) {
        ...
        usePricingModalStore.getState().openPricingModal({
          isAlert: true,
          alertTitle: 'Upgrade to access Mira Advanced mode',
        });
      }
    }
