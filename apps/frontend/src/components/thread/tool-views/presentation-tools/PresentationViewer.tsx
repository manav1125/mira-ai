...
import { fetchPresentationMetadata, sanitizePresentationName } from '../utils/presentation-utils';
...
export function PresentationViewer({
  ...
  const { isOpen, presentationName, sandboxUrl, sandboxId, initialSlide, openPresentation, closePresentation } = usePresentationViewerStore();
  ...
  const loadMetadata = useCallback(async (retryCount = 0, maxRetries = Infinity, forceRefresh = false) => {
    ...
    const sanitizedPresentationName = sanitizePresentationName(extractedPresentationName);
  ...
  }, [extractedPresentationName, project?.sandbox?.sandbox_url, project?.sandbox?.id, ensureSandboxActive, session?.access_token]);
  ...
  useEffect(() => {
    ...
    const sanitizedName = extractedPresentationName ? sanitizePresentationName(extractedPresentationName) : null;
  ...
  useEffect(() => {
    ...
    if (isStreaming && presentationToLoad && project?.sandbox?.sandbox_url) {
      ...
      const pollMetadata = async () => {
        try {
          const data = await fetchPresentationMetadata({
            presentationName: presentationToLoad,
            sandboxUrl: project?.sandbox?.sandbox_url,
            sandboxId: project?.sandbox?.id,
            accessToken: session?.access_token,
          });
          const sanitizedName = sanitizePresentationName(presentationToLoad);
  ...
  }, [isStreaming, streamingPresentationName, extractedPresentationName, project?.sandbox?.sandbox_url, project?.sandbox?.id, session?.access_token]);
  ...
  const handleDownload = async (setIsDownloading: (isDownloading: boolean) => void, format: DownloadFormat) => {
    ...
    try{
      ...
      const sanitizedName = sanitizePresentationName(extractedPresentationName);
                  ...
                  <PresentationSlideCard
                    ...
                    onFullScreenClick={(slideNumber) => {
                      if (openPresentation && project?.sandbox?.sandbox_url && (extractedPresentationName || streamingPresentationName)) {
                        openPresentation(
                          extractedPresentationName || streamingPresentationName!,
                          project.sandbox.sandbox_url,
                          slideNumber,
                          project.sandbox.id
                        );
