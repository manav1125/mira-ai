...
async def get_all_kortix_templates(
    ...
    try:
        ...
        try:
            ...
            error_str = f"Error of type {type(e).__name__}"
        logger.error(f"Error getting all VentureVerse templates: {error_str}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_marketplace_templates(
    ...
    is_kortix_team: Optional[bool] = Query(None, description="Filter for VentureVerse team templates"),
