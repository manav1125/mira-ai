...
| Deferred | Google/Drive auth | Verify OAuth clients, redirect URLs, scopes, and per-user token storage for Google Slides/Drive. | Backend auth URL generation returns `200` with Google host and current Render callback. Full consent/export verification is deferred to domain/OAuth cutover. |
...
2. Rerun GitHub Actions production E2E after the latest backend deploy.
3. Complete the domain/OAuth cutover, then run full Google consent and Slides export verification.
4. Run a full real Gmail OAuth two-account integration test before broadly promoting Gmail/Calendar/Drive integrations.
